...
void init_room_commands(RoomCommandMap& map, GameManager& manager) {
    ...
    map.insert("kick", [&manager](Room& room, Player& player, const Arguments& args) {
        ...
        if (args.size() != 1) {
            player.send_error_message("Invalid command usage. Example: /kick john");
            return;
        }
        ...
        auto player_to_kick = room.find_player_or_spectator(*nickname);
        if (!player_to_kick) {
            player.send_error_message("User '" + args.front() + "' is not in the room.");
            return;
        }
